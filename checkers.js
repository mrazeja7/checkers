...
function applyMove(move, currX, currY) {
  ...
  else if (move.type === 'jump')
  {
  	...
  	var index = landings.length - 1;
  	state.board[move.landings[index].x][move.landings[index].y] = state.board[currX][currY];
  	state.board[currX][currY] = null;
  }
}
